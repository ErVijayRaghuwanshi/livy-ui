...
export function getColumnType(field) {
  ...
  if (typeof t === "object") {
    return t.type || t.typeName || JSON.stringify(t);
  }
  return String(t);
}
...
function JsonTable({ data, elapsed, onClose, onMaximizeToggle, isMaximized, activeWidth = 500 }) {
  ...
  const columnTypes = useMemo(() => {
    return fields.map((field) => getColumnType(field));
  }, [fields]);
          ...
          <thead>
            <tr className="bg-(--color-bg-tertiary) select-none">
              ...
              {fields.map((field, i) => {
                ...
                const colType = columnTypes[i] || getColumnType(field);
                const typeIcon = getDataTypeIcon(colType);
                      ...
                      <div className="flex items-center gap-1.5 truncate">
                        ...
                        {typeIcon && (
                          <span 
                            ...
                            title={`${colType.toUpperCase()} · ${name}`}
                          >
                            {typeIcon}
                          </span>
                        )}
            ...
            {filteredRows.slice(startIdx, endIdx).map((row, i) => (
              ...
              >
                ...
                {fields.map((field, ci) => {
                  ...
                  const cellValue = rawVal === null ? "null" : rawVal !== undefined ? (typeof rawVal === "object" ? JSON.stringify(rawVal) : String(rawVal)) : "";
